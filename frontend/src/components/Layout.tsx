...
const Layout = ({ children }: LayoutProps) => {
  ...
  const breadcrumbs = useMemo(() => {
    ...
    const crumbs = segments.map((segment, index) => {
      const translatedSegment = breadcrumbTranslations[segment] || segment.replace(/-/g, ' ');
      return {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      >
        <div className={cn('flex h-full flex-col px-6 py-8', sidebarCollapsed && 'lg:px-3')}>
          ...
          <div className="mt-8 flex-1">
            <nav className={cn('space-y-2 text-sm font-semibold text-slate-500', sidebarCollapsed && 'space-y-4')}>
              {navigation.map((item) => {
                const isActive =
                  item.href === '/'
                    ? location.pathname === '/'
                ...
                return (
                  ...
                  >
                    <span className={cn('flex items-center gap-3', sidebarCollapsed && 'lg:gap-0')}>
                      <item.icon className="h-5 w-5 shrink-0" />
                      {!sidebarCollapsed && <span>{item.name}</span>}
                    </span>
                    {!sidebarCollapsed && isActive && <span className="h-2 w-2 rounded-full bg-emerald-400" />}
                  </Link>
                );
              })}
            </nav>
        ...
        <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
          <div className="flex items-center justify-between px-4 py-4 sm:px-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                className="hidden lg:flex items-center justify-center rounded-2xl border border-slate-200 p-2 text-slate-600 transition hover:bg-slate-50 hover:text-slate-900"
                onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
                title={sidebarCollapsed ? 'باز کردن منو' : 'بستن منو'}
              >
                {sidebarCollapsed ? (
