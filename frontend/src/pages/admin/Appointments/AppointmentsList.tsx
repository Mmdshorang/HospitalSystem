    ...
    <div className="space-y-8">
      <section className="rounded-[32px] border border-slate-100 bg-white px-8 py-10 shadow-sm shadow-slate-200/60">
        <div className="flex flex-wrap items-center justify-between gap-6">
          <div className="space-y-3">
            ...
            <h2 className="text-3xl font-black text-slate-900">نوبت‌ها و درخواست‌ها</h2>
      ...
      <section className="rounded-[28px] border border-slate-100 bg-white/90 p-6 shadow-sm shadow-slate-200/50">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
          ...
          <select
            value={clinicFilter || ''}
            onChange={(e) => setClinicFilter(e.target.value ? Number(e.target.value) : null)}
            className="h-12 rounded-2xl border border-slate-200 bg-white px-4 text-sm outline-none focus:border-primary"
          >
                    ...
                    <tr key={appointment.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {appointment.preferredTime
                          ? new Date(appointment.preferredTime).toLocaleDateString('fa-IR')
                          : '-'}
                      </td>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-6">
                <div className="text-sm text-gray-500">
                  صفحه {page} از {totalPages} ({appointmentsData?.totalCount || 0} مورد)
                </div>
      ...
      {selectedAppointment && (
        <>
          <ChangeStatusDialog
            ...
            onSubmit={(dto) => changeStatus.mutate({ id: selectedAppointment.id, dto })}
          />
          ...
          <AssignPerformerDialog
            ...
            appointment={selectedAppointment}
            onSubmit={async (dto) => {
              await serviceRequestService.assignPerformer(selectedAppointment.id, dto);
              queryClient.invalidateQueries({ queryKey: ['appointments'] });
